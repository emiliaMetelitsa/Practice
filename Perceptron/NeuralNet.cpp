...
double NeuralNet::DiffReLU(double x) {
	if (x > 0) { return 1; }
...
double NeuralNet::ForwardProp(int size, std::vector<double>& data) {
	...
	for (int n = 0; n < size; n++) {
		...
		neurons[n][1][0] = ReLU(neurons[n][0][0]);
	}
	...
	neurons[0][0][layers - 1] = last_sum*100;
	neurons[0][1][layers - 1] = last_sum*100;
...
void NeuralNet::BackProp(int epochs, double nu, std::vector<std::vector<double>>& data, std::vector<double>& ans){
	for (int e = 0; e < epochs; e++) {
		for (int d = 0; d < data.size(); d++) {
			double res = ForwardProp(2, data[d]);

			neurons[0][2][layers - 1] = res - ans[d];
			for (int l = layers - 2; l >= 0; l--) {
				for (int n = 0; n < neuronsInLayers[l]; n++) {
					neurons[n][2][l] = 0;
					double Diff = DiffReLU(neurons[n][1][l]);
					for (int nextN = 0; nextN < neuronsInLayers[l + 1]; nextN++) {
						neurons[n][2][l] += Diff * neurons[nextN][2][l + 1] * weights[n][nextN][l];
